...
: Node("read_write_node")
{
  ...
  set_position_subscriber_ =
    ...
    [this](const SetPosition::SharedPtr msg) -> void
    {
      ...
      if ((uint8_t) msg->id == 12 or (uint8_t) msg->id == 13){
        return;
      }
  ...
  set_position_five_motor_subscriber_ =
...
void ReadWriteNode::publishData()
{
    ...
    // 直接メンバーに値を設定
    uint8_t motor_ids[] = {10, 11, 12, 13, 14};
    int32_t positions[5];
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (dxl_comm_result == COMM_SUCCESS) {
            positions[i] = static_cast<int32_t>(present_position);
        } else {
            positions[i] = 0;  // エラー時のデフォルト値
        }
    }
    
    // メッセージに直接設定
    message.id_1 = motor_ids[0];
    message.id_2 = motor_ids[1];
    message.id_3 = motor_ids[2];
    message.id_4 = motor_ids[3];
    message.id_5 = motor_ids[4];
    
    message.position_1 = positions[0];
    message.position_2 = positions[1];
    message.position_3 = positions[2];
    message.position_4 = positions[3];
    message.position_5 = positions[4];
...
void ReadWriteNode::publishCurrentData()
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (dxl_comm_result == COMM_SUCCESS) {
            // 2の補数を考慮した変換
            // Present Currentは16bitの符号付き整数として扱う
            int16_t signed_current = static_cast<int16_t>(present_current & 0xFFFF);
            currents[i] = static_cast<int32_t>(signed_current);
        } else {
            currents[i] = 0;  // エラー時のデフォルト値
        }
    }
    ...
    message.position_1 = currents[0];
    message.position_2 = currents[1];
    message.position_3 = currents[2];
    message.position_4 = currents[3];
    message.position_5 = currents[4];
    
    publisher_five_motor_present_current_->publish(message);
}
...
{
    // 各モーターを個別に設定
    uint8_t motor_ids[] = {10, 11, 12, 13, 14, 15, 16};
    
    for (int i = 0; i < 7; i++) {
        uint8_t current_id = motor_ids[i];
        ...
        // 動作モード設定
        if (current_id == 16) {  // 特別な制御が必要なモーター
            dxl_comm_result = packetHandler->write1ByteTxRx(
                portHandler,
                current_id,
                ADDR_OPERATING_MODE,
        ...
        // プロファイル速度設定
        uint32_t profile_velocity = (current_id >= 15) ? 300 : 500;
        ...
        if (dxl_comm_result != COMM_SUCCESS) {
            RCLCPP_ERROR(rclcpp::get_logger("read_write_node"), 
                        "Failed to setup motor ID: %d", current_id);
        } else {
            RCLCPP_INFO(rclcpp::get_logger("read_write_node"), 
                       "Successfully setup motor ID: %d", current_id);
        }
    }
}
...
{
    rclcpp::init(argc, argv);  // 最初にRCLを初期化
